 ...
 *  <p><pre><code> const grants = { role: 'role1', resource:
 * 'resource1', action: 'create:any', attributes: [ attrs ] }
 *  const same = { role: 'role1', resource: 'resource1',
 * action: 'create', possession: 'any', attributes: [ attrs ] }</code></pre></p>
 *
...
class AccessControl {
  ...
  removeRoles(roles: string | string[]): AccessControl {
    ...
    utils.eachRole(this._grants, (roleItem: any, _: string) => {
   ...
   *  // Note: when multiple roles checked, acquired attributes are union-ed (merged).
   */
   ...
   */
  _removePermission(
    resources: string | string[], roles?: string | string[], actionPossession?: string,
  ) {
    ...
    utils.eachRoleResource(this._grants, (role: string, resource: string, _: any) => {
