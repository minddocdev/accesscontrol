...
const utils = {
   ...
   *  @param thisArg
   */
  each(array: any[], callback: any, thisArg = null) {
   ...
   *  @param thisArg
   */
  eachKey(object: any, callback: any, thisArg = null) {
  ...
  eachRole(grants: any, callback: (role: any, roleName: string) => void) {
   ...
   */
  eachRoleResource(
    grants: any, callback: (role: string, resource: string, resourceDefinition: any) => void,
  ) {
    let resources;
  ...
  getResources(grants: any): string[] {
    ...
    const resources: any = {};
    utils.eachRoleResource(grants, (_: string, resource: string, __: any) => {
  ...
  normalizeActionPossession(
    info: IQueryInfo | IAccessInfo, asString: boolean = false,
  ...
  normalizeQueryInfo(query: IQueryInfo): IQueryInfo {
    ...
    newQuery = utils.normalizeActionPossession(newQuery) as IQueryInfo;
    return newQuery;
  },
  ...
  normalizeAccessInfo(access: IAccessInfo, all: boolean = false): IAccessInfo {
    ...
    if (all) {
      newAccess = utils.normalizeActionPossession(newAccess) as IAccessInfo;
    }
  ...
  getRoleHierarchyOf(grants: any, roleName: string, rootRole?: string): string[] {
    ...
    role.$extend.forEach((exRoleName: string) => {
      ...
      const ext: string[] = utils.getRoleHierarchyOf(grants, exRoleName, rootRole || roleName);
  ...
  extendRole(grants: any, roles: string | string[], extenderRoles: string | string[]) {
    ...
    roles.forEach((roleName: string) => {
      ...
      const crossInherited: string = utils.getCrossExtendingRole(grants, roleName, arrExtRoles);
      if (crossInherited) {
        throw new AccessControlError(
  ...
  getUnionAttrsOfRoles(grants: any, query: IQueryInfo): string[] {
    ...
    roles.forEach((roleName: string, _: number) => {
      ...
      if (resource) {
        ...
        attrsList.push(
          (resource[`${query.action}:${query.possession}` as any]
            || resource[`${query.action}:any` as any]
            || [] as any).concat(),
        );
  ...
  lockAC(ac: AccessControl) {
    ...
    if (!locked) locked = Boolean(utils.deepFreeze(acCopy._grants));

    /* istanbul ignore next */
    if (!locked) {
      throw new AccessControlError(`Could not lock grants: ${typeof acCopy._grants}`);
    }
